...
let unfold (configuration: string) (nums: int list) =
    let repeatedNums = List.concat (List.replicate 5 nums)

    let repeatedConfig =
        [ 1..4 ]
        |> List.fold (fun acc _ -> acc + "?" + configuration) configuration

    { Configuration = repeatedConfig
      Numbers = repeatedNums }
...
let solve =
    ...
    input
    |> List.map parse
    |> List.map (fun x -> unfold x.Configuration x.Numbers)
